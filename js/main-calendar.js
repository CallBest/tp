...
function plotDates(monthval,yearval) {
  ...
  document.getElementById('main-monthbox').innerHTML = monthString(monthval) + '<br>' + yearval.toString();
  //get 42 boxes
  var boxes = document.getElementsByClassName('main-datebox');
...
function addStyleDisabled() {
  ...
  for (x=0; x < 42; x++) {
    boxes[x].classList.remove('main-date-disabled');
  }
  //falls on same month
  if ((monthval == now.getMonth()) && (yearval == now.getFullYear())) {
    for (x=i; x < dayval-1; x++) {
      boxes[x].classList.add('main-date-disabled');
    }
  }
